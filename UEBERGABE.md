...
Stand: **v1.5.205** · index.html 2,33 MB · 671 Funktionen
...
### 0m.1 · Gießmenge — entschieden und umgesetzt (v1.5.200–205)
...
Plan (203), eine Waage-Skala (204), Gießmenge aus dem Topf (205). Die Fragen (a)–(e) sind damit beantwortet: fester Gießpunkt
...
**Noch offen, in dieser Reihenfolge** (Plan der Prüfung, Schritte 9–12):
1. **Reserve als Zeitpunkt-Vorhersage** (`drybackForecast`, Statusbox an Nicht-Gießtagen): Tagesabnahme aus dem letzten
   vollständigen Zyklus; liegt der Topf morgen unter der Stressgrenze (Erde 25, Coco 60), heißt es „heute gießen". Kein
   gleitender Gießpunkt. Im Browser mit Einsteiger und Profi prüfen.
2. **Eine Klimafunktion:** `klimaTranspiration` ersetzt `_vpdFactorForDay` im alten Weg (Spülen, Outdoor) und in
   „Klima-justiert" (`_vpdAdjusted`), nur als Verhältnis.
3. **Restliche Texte:** Einstellungen zeigen V als Topf-Eigenschaft und fragen nach dem gefüllten Volumen (aufgehäuft 14–16 L
   sind nicht 11 L); Gieß-Fahrplan „Startwert ~X ml" statt „Ideal min–max"; Drain-Feld „Stofftopf/Air-Pot: in einer breiten
   Wanne auffangen"; `drainFlow` 25–30 % „viel, noch gültig"; Tipps-Leitfaden ohne feste „11 L … 3,0–3,5 Liter";
   Einsteiger-Satz mit einer ml-Zahl und der Drain-Spanne; Hinweis „Topf war noch feucht" bei Hebe-Test ≥ 70 plus Guss.
4. **ANBAU.md** 1.1 („Wann und wie viel"), 1.2 (Gießpunkt als Hebe-Skala mit Anker), 5.1 (Drain-Ziel mit Belegen), 7.5
   (Wasserhaltekapazität, S = 300 ml/L als Konvention), 7.4 (Poorter 2012), 2.3 (Oren), 14, 15. Der Text liegt in
   `scratchpad/agenten3/giessmenge_pruefung2.json` unter `anbau_md_final`.
...
Gießen) — dann ist S gemessen statt angenommen.
...
**Noch offen, in dieser Reihenfolge:** (1) Die Outdoor-VPD-Etiketten rechnen noch mit den alten Bändern — gehört in den
Outdoor-Bereich. (2) Der Klimafaktor der Gießmenge nach Oren (mit dem Gießmengen-Umbau, bis dahin `_vpdFaktorBand`). Die Befunde unten beschreiben den Stand vor v1.5.187.
...
1. **Gießmenge: die Restschritte in 0m.1** (Reserve als Zeitpunkt-Vorhersage, eine Klimafunktion, restliche Texte, ANBAU.md).
...
107 Testdateien (Gesamtlauf auf v1.5.187 grün, beide Zeitzonen, 180 Läufe; seit v1.5.188 die betroffenen) — neu dazu
