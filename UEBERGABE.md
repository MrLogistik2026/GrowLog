...
Stand: **v1.5.293** · index.html 2,45 MB · 751 Funktionen
...
**Stand Hebel 3 (Speicher-Störfälle).** Vorgezogen, weil ein stiller Totalverlust schwerer wiegt als die Hebel-1-Reste.
Unterlagen: `.claude/notizen/hebel3/plan.md` (23 gemessene Störfälle, 14 Entwürfe, je mit dem Urteil des Skeptikers),
`.claude/notizen/hebel3/patches/` (die Volltexte der Entwürfe, aus dem vergänglichen Sitzungsordner gesichert).
Abgearbeitet wird die Liste in `.claude/notizen/REIHENFOLGE.md` von oben.

- **Punkt 1 erledigt (v1.5.288 und v1.5.289):** Zwei Kopie-Generationen, Umfang nach Zyklen · Datums-Schlüsseln ·
  Tagebuch-Einträgen, der Hauptstand geht bei vollem Speicher vor den Kopien, beide Kopien sichtbar und einzeln ladbar.
  Test `test_kopien.js` (68 Prüfungen, beide Zeitzonen; 23 fallen auf v1.5.287 um, 6 auf v1.5.288). Die drei Störfälle
  wurden vorher an v1.5.287 nachgemessen und traten unverändert auf — die Messung aus dem Plan (v1.5.271) gilt weiter.
- **Wichtig für die nächsten Punkte — drei Prüfer haben v1.5.288 vor dem Hochladen zerlegt und fünf Fehler gemessen,
  zwei davon schlechter als der Vorgänger** (behoben in v1.5.289, Einzelheiten im CHANGELOG). Der Kern: Die Regel war
  richtig, ihre *Reihenfolge* nicht — „erst die zweite Generation opfern" war eine Annahme darüber, wo das Wertvolle
  liegt, und sie stimmt genau dann nicht, wenn der Schutz gebraucht wird. **Wo etwas weichen muss, entscheidet der
  Inhalt, nicht der Platz in der Liste**, und eine Aufräumaktion, die nichts bringt, muss sich zurücknehmen lassen.
  Für Punkt 3 (Rettungsplatz, Speichersperre) gilt dasselbe Muster — dort wird ebenfalls etwas aufgehoben, verdrängt
  und gelöscht. Vor dem Hochladen wieder gegenprüfen lassen.
- **Offen aus Punkt 1, bewusst zurückgestellt:** Hält die zweite Kopie einen größeren Stand fest, sagt das bisher nur die
  Karte in den Einstellungen und der zugeklappte Kopf. Die vom Skeptiker vorgeschlagene Rückfrage beim Start („Deine Daten
  sind deutlich weniger als in der Kopie vom … — War das Absicht?") braucht den Start-Hinweis aus Punkt 3 und gehört
...
- **Punkte 2 und 3 erledigt (v1.5.290):** Ein unbrauchbarer Hauptstand wird aufgehoben (`growsmart_v4_rettung`), die
  ...
  Test `test_rettung.js` (93 Prüfungen, beide Zeitzonen; 21 fallen auf v1.5.289 um, 11 auf v1.5.290, 8 auf v1.5.291, 7 auf v1.5.292).
- **Die Prüferrunden haben an jeder dieser Versionen echte Fehler gefunden — drei Runden, jedes Mal vor dem
  Hochladen.** Für alles Weitere an dieser Stelle gilt: nach dem Bauen prüfen lassen, erst dann hochladen.
  Zwei Lehren stehen im CHANGELOG ausführlich, hier in kurz: **Jede Sperre braucht eine Tür** (v1.5.291 —
  der Schutz blockierte den Rettungsweg, den die App selbst empfahl), und **bei Sicherungen gilt das Jüngste
  zuerst, der Inhalt ist nur die Notbremse** (v1.5.292 — eine Auswahl nach Inhalt holte gelöschte Zyklen
  zurück; Löschen ist meistens Absicht).
- **v1.5.291 räumt sieben Befunde der Prüfer an v1.5.290 ab**, vier davon schlechter als der Vorgänger,
  **v1.5.292 vier weitere an v1.5.291**, zwei davon schwer (Einzelheiten im CHANGELOG). Die Lehre gehört in
  jede weitere Speicher-Version: **Jede Sperre braucht eine Tür.** Der Schutz hielt das Speichern an und blockierte damit genau den Weg, den die App selbst empfahl —
  der Import kam nur in den Arbeitsspeicher und meldete trotzdem „Geladen ✓". Dazu: Eine Meldung, die in
  derselben Zeile wohnt wie die nächste Erfolgsmeldung, ist keine Meldung (jetzt ein stehendes rotes Band).
- **Punkt 4 und 5 der Reihenfolge sind damit teilweise vorweggenommen:** `_standMangel` prüft die Form
  (`cycles` Liste, `entries` Objekt), ein beschädigter Stand nimmt den Weg über Kopie und Rettungsplatz, und
  der inhaltsreichste Ersatz gewinnt. **Offen bleibt dort:** `plants` als Objekt/Text erkennen, Zyklen ohne
  `id` (sonst räumt `saveS` alle cycleData weg), leere Stellen in Listen entfernen statt Absturz.
- **Offen aus Punkt 3:** Der Rückfall „Fassung ohne Fotos aufheben, statt zu sperren" (Skeptiker-Korrektur zu
  ...
  Kommt mit Punkt 5/6.
- **Aus der Schlussprüfung, bewusst nicht gebaut:** Ein **lesbarer, aber leerer** Hauptstand neben einer vollen
  Kopie bleibt beim Start stumm — der Hauptstand gewinnt immer, und nur die Karte in den Einstellungen sagt
  „Die Sicherungskopie vom … enthält deutlich mehr". Genau diese Lage hinterlässt ein Gerät, auf dem v1.5.287
  einmal leer gestartet ist. Ein Hinweis dafür ist dieselbe Rückfrage („War das Absicht?"), die schon aus
  Punkt 1 offen ist, und gehört mit ihr zusammen gebaut. Die Kopie geht nicht verloren: Sie rückt beim
  nächsten Speichern in die zweite Generation und bleibt dort stehen (gemessen).
- **Ebenfalls gemessen und so gewollt:** Fehlt der Hauptstand und liegen eine kleine Tageskopie und ein viel
  größerer `growsmart_v3` nebeneinander, lädt v1.5.293 die Kopie (v1.5.287 lud den Altbestand). Das ist der
...
Mit `--parallel 5` waren am 17.09.2026 zusätzlich `test_dialog_und_namen` und `test_saettigungnachholen` rot und
`test_dauerdruecken` trotz `EINZELN` — auf dem neuen und dem alten Stand einzeln grün. Bis das geklärt ist: mit
`--parallel 4` laufen lassen und rote Läufe einzeln gegenprüfen, auf beiden Ständen.
